...
/**
 * Igual que /bd-capex/comparar-cierre, pero para el Presupuesto OPEX. A diferencia de
 * CAPEX, OPEX no genera un archivo nuevo por cada cierre — el archivo en vivo se
 * RENOMBRA en el lugar (ver /api/opex/mes-cierre), así que casi siempre solo va a haber
 * una opción en "Archivo": el punto de referencia real está en las VERSIONES de
 * SharePoint de ese mismo archivo, de cuando todavía tenía otro nombre de cierre.
 */
export default function CompararCierreOpex() {
  ...
  useEffect(() => {
    fetch("/api/opex/cierres-disponibles", { cache: "no-store" })
      .then((r) => r.json())
      .then((j) => {
        if (j.error) throw new Error(j.error);
        setArchivos(j.disponibles);
        setArchivoActualNombre(j.archivoActual);
        // Por defecto, el primer archivo que NO es el actual (si hay alguno) — si no hay
        // ninguno (lo más común en OPEX, un solo archivo en vivo), se usa el archivo en
        // vivo mismo, comparado contra una versión anterior de SharePoint.
        const sugerido =
          (j.disponibles as ArchivoParaComparar[]).find((a) => !a.esArchivoActual) ??
          (j.disponibles as ArchivoParaComparar[]).find((a) => a.esArchivoActual);
        if (sugerido) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/opex/versiones-archivo?itemId=${encodeURIComponent(itemIdSel)}`, { cache: "no-store" })
      .then((r) => r.json())
      .then((j) => {
        if (j.error) throw new Error(j.error);
  ...
  async function comparar() {
    ...
    try {
      ...
      const res = await fetch(`/api/opex/comparar-cierre?${params.toString()}`, { cache: "no-store" });
    ...
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="text-sm" style={{ color: "var(--texto-suave)" }}>
          Compara el Presupuesto OPEX en vivo ({archivoActualNombre || "…"}) contra una versión anterior de
          SharePoint del mismo archivo — útil cuando el archivo que se cerró siguió editándose después, y su
          contenido más reciente ya no es el que se cerró ese día.
        </p>
      </div>
        ...
        <>
          ...
          {resultado.proyectosNuevos.length > 0 && (
            <div className="card p-4 text-sm">
              <p className="font-semibold mb-1">{resultado.proyectosNuevos.length} línea(s) nueva(s) desde entonces</p>
              <p className="mb-3" style={{ color: "var(--texto-suave)" }}>
                ...
                "antes" con qué compararlos) — pero sí están contadas en el total "Ahora" de su grupo, arriba.
              ...
              <div className="overflow-x-auto">
                <table className="text-xs border-collapse w-full">
                  <thead>
                    <tr className="text-left" style={{ color: "var(--texto-suave)" }}>
                      <th className="py-1.5 px-3 font-semibold">Línea</th>
          ...
          {resultado.cambios.length === 0 ? (
            <div className="card p-6 text-center" style={{ color: "var(--exito)" }}>
              Ninguna línea cambió (Presupuesto Aprobado, Real ni Proyectado) desde ese punto de referencia.
            </div>
          ) : (
              ...
              <div style={{ maxHeight: "65vh", overflow: "auto" }}>
                <table className="text-xs border-collapse w-full">
                  <thead style={{ position: "sticky", top: 0, background: "var(--bg)" }}>
                    <tr className="text-left" style={{ color: "var(--texto-suave)" }}>
                      <th className="py-2 px-3 font-semibold">Línea</th>
